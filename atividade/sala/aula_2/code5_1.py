...
hist_n_eq = exposure.histogram(image_aerial)
plt.show()

# realizar a equalização do histograma
image_eq = exposure.equalize_hist(hist_n_eq)

# calcular e mostrar o histograma da imagem equalizada
hist_eq = exposure.equalize_hist(image_eq)
plt.show()
...
mean_pixel_low_freq_n_eq = np.max(low_freq_region_n_eq[0])

# determinar a media do valor dos pixels que ocorrem com menor frequência da imagem equalizada
mean_pixel_low_freq_eq = np.max(low_freq_region_eq[0])
